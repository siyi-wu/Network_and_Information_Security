...
本报告深入研究了基于积分梯度（Integrated Gradients, IG）的对抗样本检测与防御方法，通过量化评估和可视化分析，系统性地探讨了攻击向量的可迁移性、防御方案的鲁棒性边界、边缘设备部署挑战以及伦理安全问题。实验结果表明，IG检测器能够有效识别对抗样本，在保持较高检测率的同时，引入了可接受的计算开销。本研究为实际部署中的对抗样本防御提供了理论依据和实践指导。
...
**理论分析**：
对抗样本的跨模型可迁移性源于不同深度神经网络共享相似的决策边界特征。实验中观察到：

1. **替代模型迁移攻击**：
   - 在替代模型（ResNet20）上生成的对抗样本
   - 迁移到目标模型（相同架构）的攻击成功率：>70%
   - 迁移到不同架构（如ViT）的攻击成功率：约50-60%
...
| 指标 | 数值 | 说明 |
|------|------|------|
| 原始干净样本精度 | 92.00% | 模型在干净样本上的基线性能 |
| 防御后干净样本精度 | 87.00% | 检测器引入的精度损失：5.00% |
| 对抗攻击成功率 | 85.60% | 原始模型鲁棒性：14.40% |
| 防御后对抗样本精度 | 72.40% | 鲁棒性提升：58.00% |
| 基础推理延迟 | 2.45 ms | 单张样本推理时间 |
| 防御后推理延迟 | 3.72 ms | 延迟开销：1.52倍 |
...
| ε值 | 攻击成功率 | 防御后精度 | 检测率 |
|-----|-----------|-----------|--------|
| 2/255 | 45.20% | 88.60% | 96.80% |
| 4/255 | 68.40% | 82.30% | 94.20% |
| 8/255 | 85.60% | 72.40% | 91.50% |
| 12/255 | 92.10% | 65.80% | 87.30% |
| 16/255 | 95.80% | 58.20% | 82.60% |

**关键发现**：

1. **防御有效性随攻击强度递减**：
   - 低强度攻击（ε≤4/255）：检测率>94%
   - 中等强度攻击（ε=8/255）：检测率≈91.5%
   - 高强度攻击（ε≥12/255）：检测率<88%

2. **鲁棒性边界识别**：
   - 最佳防御区间：ε∈[4/255, 8/255]
   - 边界点：ε≈10/255（检测率降至90%以下）
   - 失效阈值：ε>16/255（检测率<85%）
...
3. **权衡关系量化**：
   ...
   在ε=8/255时：
   - 防御增益 = 72.40% - 14.40% = 58.00%
   - 精度损失 = 92.00% - 87.00% = 5.00%
   - 效能比 = 11.6（最优）
...
| 攻击强度 | LPIPS | SSIM | 检测率 |
|---------|-------|------|--------|
| 2/255 | 0.0234 | 0.9856 | 96.80% |
| 4/255 | 0.0456 | 0.9712 | 94.20% |
| 8/255 | 0.0892 | 0.9428 | 91.50% |
| 12/255 | 0.1345 | 0.9156 | 87.30% |
| 16/255 | 0.1789 | 0.8834 | 82.60% |

**分析结论**：

1. **感知质量与检测率正相关**：
   - 高感知质量（LPIPS<0.05）→ 高检测率（>94%）
   - 中等感知质量（0.05<LPIPS<0.1）→ 中等检测率（90-94%）
   - 低感知质量（LPIPS>0.1）→ 低检测率（<90%）

2. **防御边界与人类感知一致**：
   - 人眼难以察觉的扰动（LPIPS<0.05）能被有效检测
   - 明显可见的扰动（LPIPS>0.1）检测率下降
   - 表明IG检测器与人类感知具有某种一致性
...
1. **积分步数优化**：
   - n_steps=20：1.27 ms（基线）
   - n_steps=10：0.68 ms（检测率下降2-3%）
   - n_steps=5：0.35 ms（检测率下降5-8%）
