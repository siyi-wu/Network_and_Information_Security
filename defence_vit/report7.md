...
|---------|------|-----------|-----------|----------|
| 图像分类 | ViT-Tiny | 88.5% | 13.2% | 75.3% |
| 目标检测 | Faster R-CNN | 42.3% (mAP) | 38.7% (mAP) | 8.5% |

**分析**：
- 对抗样本对分类任务的攻击效果显著（精度下降75.3%）
...
|---------|----------|-----------|----------------|-------|------|
| 1 | 12.3% | 8.5% | 5.2% | 0.02 | 0.98 |
| 2 | 28.7% | 18.3% | 11.5% | 0.04 | 0.95 |
| 4 | 52.4% | 35.6% | 22.8% | 0.08 | 0.89 |
| 6 | 68.9% | 48.2% | 31.5% | 0.12 | 0.82 |
| 8 | 78.5% | 55.3% | 38.7% | 0.16 | 0.76 |
| 12 | 85.2% | 62.1% | 45.6% | 0.24 | 0.65 |
| 16 | 89.7% | 68.4% | 52.3% | 0.32 | 0.54 |
| 20 | 92.1% | 73.5% | 58.9% | 0.41 | 0.43 |
| 24 | 94.3% | 77.8% | 64.2% | 0.51 | 0.32 |

#### 4.1.3 鲁棒性边界分析

**关键发现**：
1. **防御有效性递减**：随着ε增大，防御效果逐渐减弱
   - ε=4时：PSM+对抗训练将ASR从52.4%降至22.8%（降低56.5%）
   - ε=24时：PSM+对抗训练将ASR从94.3%降至64.2%（降低31.9%）
...
3. **防御边界**：
   - **安全边界**：ε≤4，防御后ASR<25%，感知质量良好
   - **风险边界**：ε>12，防御后ASR>45%，感知质量下降明显
   - **失效边界**：ε>20，防御后ASR>55%，防御效果有限
...
|---|----------|----------|----------|-----------|
| 1.0（仅干净） | 88.5% | 13.2% | 0% | 0% |
| 0.8 | 87.2% | 28.5% | 1.3% | 116% |
| 0.6 | 85.8% | 38.7% | 2.7% | 193% |
| 0.4 | 83.2% | 45.6% | 5.3% | 245% |
| 0.2（仅对抗） | 78.5% | 52.3% | 10.0% | 296% |
...
2. **精度-鲁棒性曲线**：
   - α∈[0.6, 0.8]：精度损失<3%，鲁棒性提升>116%
   - α<0.4：精度损失>5%，鲁棒性提升边际递减
...
|---------|--------|-------|----------|----------|
| 基线ViT | 5.7M | 0.46G | 12.3ms | 0% |
| PSM-ViT | 5.9M | 0.48G | 14.8ms | 20.3% |
| PSM+对抗训练ViT | 5.9M | 0.48G | 14.8ms | 20.3% |
...
|------|--------|-------|----------|----------|----------|
| 原始PSM+ViT | 5.9M | 0.48G | 14.8ms | 100% | 云端服务器 |
| 知识蒸馏 | 3.0M | 0.25G | 8.2ms | 95% | 高性能边缘设备 |
| 量化+蒸馏 | 0.8M | 0.12G | 4.5ms | 90% | 中等性能边缘设备 |
| MobileViT+轻量PSM | 2.8M | 0.25G | 8.2ms | 92% | 移动设备 |
| 全栈优化 | 0.6M | 0.08G | 3.2ms | 85% | 低功耗IoT设备 |
...
1. **防御有效性**：集成PSM模块的ViT通过联合对抗训练，在保持干净样本精度85.8%的同时，将对抗样本精度从13.2%提升至38.7%，鲁棒性提升193%。
...
4. **轻量化部署**：通过知识蒸馏、量化、剪枝等技术，可将模型参数量减少90%，推理延迟降低至3.2ms，适合边缘设备部署。
