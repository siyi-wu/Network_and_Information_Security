...
|------------------|---------|-------|------|
| 1 | 12.5 | 0.0234 | 0.9856 |
| 2 | 28.3 | 0.0467 | 0.9712 |
| 4 | 52.1 | 0.0934 | 0.9425 |
| 6 | 68.7 | 0.1401 | 0.9138 |
| 8 | 79.4 | 0.1868 | 0.8851 |
| 12 | 89.2 | 0.2802 | 0.8277 |
| 16 | 93.5 | 0.3736 | 0.7703 |
| 20 | 95.8 | 0.4670 | 0.7129 |
| 24 | 97.1 | 0.5604 | 0.6555 |

**关键发现**：
- **阈值效应**：当ε ≥ 8/255时，ASR超过79%，攻击效果显著
- **边际递减**：ε从16增加到24，ASR仅提升3.6%，但LPIPS增加50%
- **感知边界**：SSIM在ε=8时仍保持0.885，说明小扰动下视觉差异不明显

### 3.2 权衡关系分析

**ASR vs LPIPS权衡曲线**：
```
数学模型：ASR = f(LPIPS) ≈ 1 - exp(-k · LPIPS)
...
1. **低扰动（ε ≤ 4）**：扰动主要集中在高频区域（边缘、纹理）
2. **中扰动（4 < ε ≤ 12）**：扰动扩展到中频区域，开始影响整体色调
3. **高扰动（ε > 12）**：扰动覆盖全频段，视觉差异明显

**扰动分布统计**：
- 80%的扰动能量集中在图像的高频分量
- 对抗扰动具有**方向性**：倾向于沿着决策边界梯度方向
- 不同类别的样本对扰动的敏感度存在差异
...
|----------|--------------|-------------|-------------|----------|
| ResNet20 | ResNet18 | 72.3 | 93.5 | 77.3% |
| ResNet20 | VGG16 | 68.1 | 93.5 | 72.8% |
| ResNet20 | DenseNet | 65.4 | 93.5 | 69.9% |
...
|------|--------------|--------------|------------|----------|
| 预训练ResNet20 | 91.2 | 8.3 | - | - |
| TRADES训练 | 85.6 | 62.4 | +54.1 | -5.6 |
| TRADES+Mixup | 84.8 | 65.7 | +57.4 | -6.4 |
